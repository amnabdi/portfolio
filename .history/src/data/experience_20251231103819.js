import sibitecLogo from "../assets/images/sibitec_logo.svg";
import freelancerLogo from "../assets/images/freelancer.png";

export const experiences = [
  {
    id: 1,
    company: "Simorgh Intelligent Business Ideas",
    role: "Frontend Developer",
    location: "Tehran, Iran",
    link: "https://sibitec.ir",
    duration: "2023 - Present",
    period: "2y 5mos",
    logo: sibitecLogo,
    responsibilities: [
      "Modernized Iran Insurance systems with a secure Next.js PWA and micro-frontend architecture.",
      "Built a scalable insurance platform with modular architecture and dynamic form/PDF generation.",
      "Led Amro PWA with wallet, OTP, AI assessment, and admin panel.",
      "Developed a React design system with customizable MUI components.",
      "Rebuilt Simorgh’s landing page in Next.js 14 with CMS-driven layouts.",
      "Built and managed Sibitec CMS for Simorgh, enabling dynamic content management.",
      "Created a React-based user management system with roles and access control.",
      "Replaced Apex Holding’s legacy panel with a modern React interface.",
    ],
  },
  {
    id: 2,
    company: "Freelancer",
    role: "Frontend Developer",
    location: "Remote",
    link: "/",
    duration: "2022 - 2023",
    period: "1y",
    logo: freelancerLogo,
    responsibilities: [
      "Developed a React-based user management system with roles and access control.",
      "Replaced Apex Holding’s legacy panel with a modern React interface.",
    ],
  },
];
